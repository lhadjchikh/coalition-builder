  ...
  return function MockSocialShareButtons(props: any) {
    return (
      <div data-testid="social-share-buttons">
        <button onClick={() => props.onShare?.('facebook')}>Share on Facebook</button>
...
describe('EndorsementForm', () => {
  ...
  describe('Social Sharing Integration', () => {
    it('displays social share buttons after successful submission', async () => {
      ...
      fireEvent.change(screen.getByTestId('state-select'), { target: { value: 'IL' } });
      fireEvent.change(screen.getByTestId('zip-code-input'), { target: { value: '62701' } });
      ...
      await waitFor(() => {
        expect(screen.getByText('Thank you for your endorsement!')).toBeInTheDocument();
      });
    ...
    it('passes correct props to SocialShareButtons component', async () => {
      ...
      expect(
        screen.getByText(`URL: ${window.location.origin}/campaigns/${mockCampaign.name}`)
      ).toBeInTheDocument();
      expect(screen.getByText(`Title: I just endorsed ${mockCampaign.title}!`)).toBeInTheDocument();
    });
    ...
    it('tracks analytics when endorsement is submitted', async () => {
      ...
      await waitFor(() => {
        expect(analytics.trackEndorsementSubmission).toHaveBeenCalledWith(mockCampaign.name);
      });
    });

    it('does not show share buttons when submission fails', async () => {
      (API.createEndorsement as jest.Mock).mockRejectedValue(new Error('Submission failed'));
    ...
    it('calls onFormInteraction callback when form gains and loses focus', () => {
      render(<EndorsementForm campaign={mockCampaign} onFormInteraction={mockOnFormInteraction} />);
  ...
  describe('Form Validation and Edge Cases', () => {
    it('displays endorsement not allowed message when campaign does not allow endorsements', () => {
      const campaignNoEndorsements = { ...mockCampaign, allow_endorsements: false };
    ...
    it('displays endorsement statement when provided', () => {
      ...
      expect(
        screen.getByText('By submitting this form, you agree to the following statement:')
      ).toBeInTheDocument();
      expect(screen.getByText('"We support this important policy change."')).toBeInTheDocument();
    });

    it('displays endorsement form instructions when provided', () => {
      const campaignWithInstructions = {
        ...mockCampaign,
        endorsement_form_instructions: '<p>Please fill out all required fields.</p>',
      };
