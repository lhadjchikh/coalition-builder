import { getFallbackHomepage, getApiUrl, fetchHomepage, fetchCampaigns } from '../homepage-data';
...
describe('homepage-data', () => {
  ...
  describe('fetchHomepage', () => {
    ...
    it('should fetch homepage successfully', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/homepage/', {
        next: { revalidate: 300 },
      });
    ...
    it('should use correct API URL from environment', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://api.example.com/api/homepage/', {
        next: { revalidate: 300 },
      });
    });
    ...
    it('should throw error when fetch fails', async () => {
      ...
      await expect(fetchHomepage()).rejects.toThrow('Failed to fetch homepage: 404');
    });
    ...
    it('should handle server error status codes', async () => {
      ...
      await expect(fetchHomepage()).rejects.toThrow('Failed to fetch homepage: 500');
    });
  });
  ...
  describe('fetchCampaigns', () => {
    ...
    it('should fetch campaigns successfully', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:8000/api/campaigns/', {
        next: { revalidate: 300 },
      });
    ...
    it('should throw error when fetch fails', async () => {
      ...
      await expect(fetchCampaigns()).rejects.toThrow('Failed to fetch campaigns: 403');
    });
    ...
    it('should handle server error status codes', async () => {
      ...
      await expect(fetchCampaigns()).rejects.toThrow('Failed to fetch campaigns: 503');
    });
  });
