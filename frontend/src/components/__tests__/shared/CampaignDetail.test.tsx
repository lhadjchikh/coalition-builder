...
jest.mock('@shared/components/SocialShareButtons', () => ({
  __esModule: true,
  default: ({ url, title, description, _hashtags, _campaignName, showLabel, className }: any) => (
...
describe('CampaignDetail Share Modal', () => {
  ...
  describe('Share Icon Overlay', () => {
    ...
    it('does not render share icon when no hero image', () => {
      ...
      render(<CampaignDetail {...defaultProps} initialCampaign={campaignWithoutImage} />);
  ...
  describe('Share Modal', () => {
    ...
    it('passes correct props to SocialShareButtons', () => {
      ...
      expect(screen.getByTestId('share-url')).toHaveTextContent(
        'http://localhost:3000/campaigns/test-campaign'
      );
      expect(screen.getByTestId('share-title')).toHaveTextContent('Test Campaign');
      expect(screen.getByTestId('share-description')).toHaveTextContent('Test campaign summary');
  ...
  describe('Integration with Campaign Data', () => {
    it('uses campaign data for share URL', () => {
      ...
      expect(screen.getByTestId('share-url')).toHaveTextContent(
        'https://example.com/campaigns/test-campaign'
      );
    });
    ...
    it('falls back to description when summary is not available', () => {
      ...
      render(<CampaignDetail {...defaultProps} initialCampaign={campaignWithoutSummary} />);

      const shareButton = screen.getByLabelText('Share this campaign');
      fireEvent.click(shareButton);

      expect(screen.getByTestId('share-description')).toHaveTextContent('Detailed description');
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('handles missing campaign name', () => {
      ...
      expect(screen.getByTestId('share-url')).toHaveTextContent('http://localhost:3000/campaigns/');
    });
