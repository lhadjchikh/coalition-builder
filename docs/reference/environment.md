...
1. **Use .env files locally** - Keep sensitive data out of your code
2. **Use cloud secret management** - AWS Secrets Manager in production
...
5. **Validate required variables** - Check for missing variables at startup
...
DATABASES = {
    'default': dj_database_url.config(
        default=f"postgresql://postgres:postgres@localhost:5432/{os.environ.get('DB_NAME', 'coalition')}"
    )
}
```
